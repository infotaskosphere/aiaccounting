...
export default function Payroll() {
  ...
  const payrollData = getCompanyData(activeCompany?.id).payroll
  ...
  const handleAdd=(emp)=>{ setEmployees(p=>[...p,emp]); setAddModal(false); toast.success(`${emp.name} added`) }
  const handleEdit=(emp)=>{ setEmployees(p=>p.map(e=>e.id===emp.id?emp:e)); setEditEmp(null); toast.success('Employee updated') }
  const handleDelete=()=>{ setEmployees(p=>p.filter(e=>e.id!==deleteEmp.id)); toast.success(`${deleteEmp.name} removed`); setDeleteEmp(null) }
