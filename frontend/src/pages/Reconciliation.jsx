...
export default function Reconciliation() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setApiError(true)
      const mockTxns = getCompanyData(activeCompany?.id).bankTransactions || []
      setTxns(mockTxns)
      const matched   = mockTxns.filter(t=>t.status==='matched').length
      const unmatched = mockTxns.filter(t=>t.status==='unmatched').length
      setSummary({
        matched,
        unmatched,
        in_review: 0,
        unmatched_amount: mockTxns.filter(t=>t.status==='unmatched').reduce((s,t)=>s+t.amount,0),
      })
    } finally {
      setLoading(false)
    }
